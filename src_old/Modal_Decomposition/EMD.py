...
Modify:  (must)
    ...
    2026.7.7  - Rebuild the EMD with abstract class. And realize the EMD independently.
"""

import numpy as npfrom typing import Tuple, Literal

from .Base import Decomposer, DecompositionResult, Config, EMDConfig
from ._Registry import register_class, register_function
from .Utils import Check_ST_and_Transform, monotonic
...
@register_class("EMD")
class EMD(Decomposer):  # TODO: 需要后续把其他用到EMD的引用到这个class上来
    name = "EMD"

    def __init__(self, nbsym: int = 2, spline_kind: Literal["akima", "cubic", "pchip", "cubic_hermite", "slinear", "quadratic", "linear"] = "cubic", max_imf: int = -1, T: list | np.ndarray = None, RAISE: bool = True):
        self.nbsym = nbsym
